assert((syn and syn.request) or (http and http.request) or http_request or (fluxus and fluxus.request) or request, "Unsupported exploit (should support syn.request or http.request)")
...
local version = "v.2.0.0";
local logname = string.format("%d-%s-log.txt", game.PlaceId, os.date("%d_%m_%y"));

if options.SaveLogs then
    writefile(logname, string.format("Eclipse HTTP SPY Logs from %s\n\n", os.date("%d/%m/%y"))) 
end;

local Serializer = loadstring(game:HttpGet("https://raw.githubusercontent.com/EclipseVSC/HTTPSpyModded/main/EclipseSerializer.lua"))();
local clonef = clonefunction;
local pconsole = clonef(print or rconsoleprint or "Not Supported");
...
local libtype = syn and "syn" or "http" or fluxus and "fluxus";
local hooked = {};
local proxied = {};
local methods = {
    HttpGet = true,
